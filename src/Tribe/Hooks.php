 ...
 * To remove a filter:
 * ```php
 *  remove_filter( 'some_filter', [ tribe( Tribe\Extensions\ET_Test_Data_Generator\Hooks::class ), 'some_filtering_method' ] );
 *  remove_filter( 'some_filter', [ tribe( 'events-virtual.hooks' ), 'some_filtering_method' ] );
 * ```
 *
 * To remove an action:
 * ```php
 *  remove_action( 'some_action', [ tribe( Tribe\Extensions\ET_Test_Data_Generator\Hooks::class ), 'some_method' ] );
...
class Hooks extends \tad_DI52_ServiceProvider {
	...
	public function register() {
		...
		$this->container->singleton( 'extension.et_test_data_generator.hooks', $this );
	...
	protected function add_actions() {
		...
		tribe_notice(
			'tec-ext-et-test-data-generator-success',
			[
			...
			[
				'type' => 'success',
				'action' => 'tec_ext_et-test_data_generator_notices'
			],
			function() {
		...
		tribe_notice(
			'tec-ext-et-test-data-generator-error',
			[
				$this->container->make( Page::class ),
				'render_error_notice'
			],
